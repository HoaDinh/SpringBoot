...
//
    ...
    public Map<String, Double> maleFemaleratio() {
        Map<String, Long>  numberPeople = people
                .stream()
                .collect(Collectors.groupingBy(Person::getCity,Collectors.counting()));
        Map<String, Double> groupDevelopersByCity = people
                .stream()
                .filter(p->p.getGender().equals("Male"))
                .collect(Collectors.groupingBy(Person::getCity,Collectors.counting())).entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey,
                        entry ->entry.getValue().doubleValue()/(numberPeople.get(entry.getKey())- entry.getValue())));
        System.out.println(groupDevelopersByCity);
        System.out.println(numberPeople);
        return groupDevelopersByCity;
    }
   // Tính mức lương trung bình của tất cả những người trên 30 tuổi
   public String avgOverThirty() {
       return "mức lương trung bình của tất cả những người trên 30 tuổi là: "+ people
               .stream()
